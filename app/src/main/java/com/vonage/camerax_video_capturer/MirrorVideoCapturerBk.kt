...
class MirrorVideoCapturerBk(
    ...
    var handler = Handler()
    private var newFrame: Runnable = object : Runnable {
        override fun run() {
            if (isCaptureRunning) {
                if (frame == null) {
    ...
    private fun findClosestEnclosingFpsRange(
        ...
        if (supportedFpsRanges.isNullOrEmpty()) {
    ...
    companion object {
        private val TAG = MirrorVideoCapturerBk::class.java.simpleName
        private const val PIXEL_FORMAT = ImageFormat.NV21
        private fun roundRotation(rotation: Int): Int {
            return ((rotation.toDouble() / 90).roundToInt() * 90).toInt() % 360
        }
